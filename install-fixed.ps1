...
function ExitWithSuccess([string]$msg) {
    if ($JsonOutput) {
        $result = @{ 
            success = $true
            message = $msg
...
# Skip execution if WhatIf is specified
if ($WhatIf) {
    Write-Host "WhatIf mode: Script syntax is valid. No actions will be performed."
    Exit 0
}
...
# Exit with success
ExitWithSuccess "Install complete"
