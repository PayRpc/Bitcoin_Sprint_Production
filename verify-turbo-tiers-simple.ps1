...
if (Test-Path "config-turbo.json") {
	...
	if ($turboConfig.PSObject.Properties.Name -contains "performance_mode") {
		Write-Host "     - performance_mode: $($turboConfig.performance_mode)" -ForegroundColor Gray
	}
}
else {
	Write-Host "   [X] config-turbo.json not found" -ForegroundColor Red
}
...
if (Test-Path "license-enterprise.json") {
	...
	Write-Host "     - turbo_mode feature: $($enterpriseLicense.license.features.turbo_mode)" -ForegroundColor Gray
}
else {
	Write-Host "   [X] license-enterprise.json not found" -ForegroundColor Red
}
...
if (Test-Path $statusApiPath) {
	...
	if ($statusContent -match "turbo_mode_enabled.*ENTERPRISE.*ENTERPRISE_PLUS") {
...
if (Test-Path $licenseInfoPath) {
	...
	if ($licenseContent -match "turbo_mode.*ENTERPRISE.*ENTERPRISE_PLUS") {
...
if (Test-Path $mainGoPath) {
	...
	if ($goContent -match "TurboMode.*bool") {
		Write-Host "   [OK] TurboMode field found in config struct" -ForegroundColor Green
	}
 else {
	...
	if ($goContent -match "TURBO_MODE") {
		Write-Host "   [OK] TURBO_MODE environment variable support found" -ForegroundColor Green
	}
 else {
	...
	if ($goContent -match "TurboMode.*rateLimiter") {
		Write-Host "   [OK] Turbo mode integrated with rate limiting" -ForegroundColor Green
	}
 else {
	...
	if ($goContent -match "TurboModeEnabled") {
		Write-Host "   [OK] Turbo mode included in status response" -ForegroundColor Green
	}
 else {
	...
	Write-Host "   [X] $mainGoPath not found" -ForegroundColor Red
}
...
try {
	...
	Write-Host "   [INFO] Go service not responding on port 8080" -ForegroundColor Yellow
}
...
try {
	...
	Write-Host "   [INFO] Web API not responding on port 3000" -ForegroundColor Yellow
}
...
Write-Host "- Run test-turbo-tiers.ps1 for comprehensive testing" -ForegroundColor White
